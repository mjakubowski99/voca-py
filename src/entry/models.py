...
from sqlalchemy import BigInteger, Boolean, CheckConstraint, Computed, Double, ForeignKeyConstraint, Index, Integer, JSON, Numeric, PrimaryKeyConstraint, Sequence, SmallInteger, String, Text, UniqueConstraint, Uuid, text
...
class Admins(Base):
    ...
    flashcard_decks: Mapped[list['FlashcardDecks']] = relationship('FlashcardDecks', back_populates='admin')
...
class Exercises(Base):
    ...
    exercise_entries: Mapped[list['ExerciseEntries']] = relationship('ExerciseEntries', back_populates='exercise')
    unscramble_word_exercises: Mapped[list['UnscrambleWordExercises']] = relationship('UnscrambleWordExercises', back_populates='exercise')
...
class PulseAggregates(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint('bucket', 'period', 'type', 'aggregate', 'key_hash', name='pulse_aggregates_bucket_period_type_aggregate_key_hash_unique'),
    ...
    key_hash: Mapped[uuid.UUID] = mapped_column(Uuid, Computed('(md5(key))::uuid', persisted=True), nullable=False)
...
class Stories(Base):
    ...
    __table_args__ = (
        PrimaryKeyConstraint('id', name='stories_pkey'),
    )
    ...
    story_flashcards: Mapped[list['StoryFlashcards']] = relationship('StoryFlashcards', back_populates='story')
...
class Users(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint('provider_id', 'provider_type', name='users_provider_id_provider_type_unique'),
    ...
    email: Mapped[str] = mapped_column(String(255), nullable=False)
    password: Mapped[str] = mapped_column(String(255), nullable=False)
    user_language: Mapped[str] = mapped_column(String(3), nullable=False, server_default=text("'pl'::character varying"))
    learning_language: Mapped[str] = mapped_column(String(3), nullable=False, server_default=text("'en'::character varying"))
    ...
    flashcard_decks: Mapped[list['FlashcardDecks']] = relationship('FlashcardDecks', back_populates='user')
    reports: Mapped[list['Reports']] = relationship('Reports', back_populates='user')
    flashcard_deck_activities: Mapped[list['FlashcardDeckActivities']] = relationship('FlashcardDeckActivities', back_populates='user')
    flashcards: Mapped[list['Flashcards']] = relationship('Flashcards', back_populates='user')
    learning_sessions: Mapped[list['LearningSessions']] = relationship('LearningSessions', back_populates='user')
    flashcard_poll_items: Mapped[list['FlashcardPollItems']] = relationship('FlashcardPollItems', back_populates='user')
    sm_two_flashcards: Mapped[list['SmTwoFlashcards']] = relationship('SmTwoFlashcards', back_populates='user')
    phone = Column(String(20))

class ExerciseEntries(Base):
    __tablename__ = 'exercise_entries'
    __table_args__ = (
        ForeignKeyConstraint(['exercise_id'], ['exercises.id'], ondelete='CASCADE', name='exercise_entries_exercise_id_foreign'),
    ...
    score: Mapped[float] = mapped_column(Double(53), nullable=False, server_default=text("'0'::double precision"))
    answers_count: Mapped[int] = mapped_column(SmallInteger, nullable=False, server_default=text("'0'::smallint"))
    order: Mapped[int] = mapped_column(SmallInteger, nullable=False, server_default=text("'0'::smallint"))
...
class FlashcardDecks(Base):
    ...
    __table_args__ = (
        CheckConstraint('admin_id IS NOT NULL AND user_id IS NULL OR admin_id IS NULL AND user_id IS NOT NULL', name='check_flashcard_decks_user_id_admin_id_both_not_null'),
        ForeignKeyConstraint(['admin_id'], ['admins.id'], name='flashcard_decks_admin_id_foreign'),
        ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE', name='flashcard_categories_user_id_foreign'),
        PrimaryKeyConstraint('id', name='flashcard_categories_pkey'),
        Index('flashcard_categories_user_id_index', 'user_id')
    )

    id: Mapped[int] = mapped_column(BigInteger, Sequence('flashcard_categories_id_seq'), primary_key=True)
    tag: Mapped[str] = mapped_column(String(255), nullable=False)
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    default_language_level: Mapped[str] = mapped_column(String(255), nullable=False, server_default=text("'B2'::character varying"))
    ...
    flashcard_deck_activities: Mapped[list['FlashcardDeckActivities']] = relationship('FlashcardDeckActivities', back_populates='flashcard_deck')
    flashcards: Mapped[list['Flashcards']] = relationship('Flashcards', back_populates='flashcard_deck')
    learning_sessions: Mapped[list['LearningSessions']] = relationship('LearningSessions', back_populates='flashcard_deck')
...
class UnscrambleWordExercises(Base):
    __tablename__ = 'unscramble_word_exercises'
    __table_args__ = (
        ForeignKeyConstraint(['exercise_id'], ['exercises.id'], ondelete='CASCADE', name='unscramble_word_exercises_exercise_id_foreign'),
        PrimaryKeyConstraint('id', name='unscramble_word_exercises_pkey'),
        Index('unscramble_word_exercises_exercise_id_index', 'exercise_id')
    )

    id: Mapped[int] = mapped_column(BigInteger, primary_key=True)
    exercise_id: Mapped[int] = mapped_column(BigInteger, nullable=False)
    word: Mapped[str] = mapped_column(String(255), nullable=False)
    scrambled_word: Mapped[str] = mapped_column(String(255), nullable=False)
    context_sentence: Mapped[str] = mapped_column(String(255), nullable=False)
    word_translation: Mapped[str] = mapped_column(String(255), nullable=False)
    emoji: Mapped[Optional[str]] = mapped_column(String(255))
    created_at: Mapped[Optional[datetime.datetime]] = mapped_column(TIMESTAMP(precision=0))
    updated_at: Mapped[Optional[datetime.datetime]] = mapped_column(TIMESTAMP(precision=0))
    context_sentence_translation: Mapped[Optional[str]] = mapped_column(String(255))

    exercise: Mapped['Exercises'] = relationship('Exercises', back_populates='unscramble_word_exercises')


class FlashcardDeckActivities(Base):
    __tablename__ = 'flashcard_deck_activities'
    __table_args__ = (
        ForeignKeyConstraint(['flashcard_deck_id'], ['flashcard_decks.id'], ondelete='CASCADE', name='flashcard_deck_activities_flashcard_deck_id_foreign'),
        ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE', name='flashcard_deck_activities_user_id_foreign'),
        PrimaryKeyConstraint('id', name='flashcard_deck_activities_pkey'),
        UniqueConstraint('user_id', 'flashcard_deck_id', name='flashcard_deck_activities_user_id_flashcard_deck_id_unique'),
    ...
    flashcard_deck: Mapped['FlashcardDecks'] = relationship('FlashcardDecks', back_populates='flashcard_deck_activities')
...
class Flashcards(Base):
    ...
    __table_args__ = (
        CheckConstraint('admin_id IS NOT NULL AND user_id IS NULL OR admin_id IS NULL AND user_id IS NOT NULL', name='check_flashcards_user_id_admin_id_both_not_null'),
        ForeignKeyConstraint(['admin_id'], ['admins.id'], name='flashcards_admin_id_foreign'),
        ForeignKeyConstraint(['flashcard_deck_id'], ['flashcard_decks.id'], ondelete='CASCADE', name='flashcards_flashcard_deck_id_foreign'),
    ...
    language_level: Mapped[str] = mapped_column(String(255), nullable=False, server_default=text("'B2'::character varying"))
    user_id: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid)
    ...
    flashcard_deck: Mapped[Optional['FlashcardDecks']] = relationship('FlashcardDecks', back_populates='flashcards')
    user: Mapped[Optional['Users']] = relationship('Users', back_populates='flashcards')
    flashcard_poll_items: Mapped[list['FlashcardPollItems']] = relationship('FlashcardPollItems', back_populates='flashcard')
    learning_session_flashcards: Mapped[list['LearningSessionFlashcards']] = relationship('LearningSessionFlashcards', back_populates='flashcard')
    sm_two_flashcards: Mapped[list['SmTwoFlashcards']] = relationship('SmTwoFlashcards', back_populates='flashcard')
    story_flashcards: Mapped[list['StoryFlashcards']] = relationship('StoryFlashcards', back_populates='flashcard')


class LearningSessions(Base):
    __tablename__ = 'learning_sessions'
    __table_args__ = (
        ForeignKeyConstraint(['flashcard_deck_id'], ['flashcard_decks.id'], ondelete='CASCADE', name='learning_sessions_flashcard_deck_id_foreign'),
    ...
    type: Mapped[str] = mapped_column(String(255), nullable=False, server_default=text("'flashcard'::character varying"))
    flashcard_deck_id: Mapped[Optional[int]] = mapped_column(BigInteger)
    created_at: Mapped[Optional[datetime.datetime]] = mapped_column(TIMESTAMP(precision=0))
    updated_at: Mapped[Optional[datetime.datetime]] = mapped_column(TIMESTAMP(precision=0))

    flashcard_deck: Mapped[Optional['FlashcardDecks']] = relationship('FlashcardDecks', back_populates='learning_sessions')
    user: Mapped['Users'] = relationship('Users', back_populates='learning_sessions')
    learning_session_flashcards: Mapped[list['LearningSessionFlashcards']] = relationship('LearningSessionFlashcards', back_populates='learning_session')


class FlashcardPollItems(Base):
    __tablename__ = 'flashcard_poll_items'
    __table_args__ = (
        ForeignKeyConstraint(['flashcard_id'], ['flashcards.id'], ondelete='CASCADE', name='flashcard_poll_items_flashcard_id_foreign'),
        ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE', name='flashcard_poll_items_user_id_foreign'),
    ...
    easy_ratings_count: Mapped[int] = mapped_column(SmallInteger, nullable=False, server_default=text("'0'::smallint"))
    easy_ratings_count_to_purge: Mapped[int] = mapped_column(SmallInteger, nullable=False, server_default=text("'0'::smallint"))
    leitner_level: Mapped[int] = mapped_column(SmallInteger, nullable=False)
    created_at: Mapped[Optional[datetime.datetime]] = mapped_column(TIMESTAMP(precision=0))
    updated_at: Mapped[Optional[datetime.datetime]] = mapped_column(TIMESTAMP(precision=0))

    flashcard: Mapped['Flashcards'] = relationship('Flashcards', back_populates='flashcard_poll_items')
...
class LearningSessionFlashcards(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['flashcard_id'], ['flashcards.id'], ondelete='CASCADE', name='learning_session_flashcards_flashcard_id_foreign'),
        ForeignKeyConstraint(['learning_session_id'], ['learning_sessions.id'], ondelete='CASCADE', name='learning_session_flashcards_learning_session_id_foreign'),
    ...
    flashcard_id: Mapped[int] = mapped_column(BigInteger, nullable=False)
    is_additional: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default=text('false'))
    ...
    flashcard: Mapped['Flashcards'] = relationship('Flashcards', back_populates='learning_session_flashcards')
    learning_session: Mapped['LearningSessions'] = relationship('LearningSessions', back_populates='learning_session_flashcards')


class SmTwoFlashcards(Base):
    __tablename__ = 'sm_two_flashcards'
    __table_args__ = (
        ForeignKeyConstraint(['flashcard_id'], ['flashcards.id'], ondelete='CASCADE', name='sm_two_flashcards_flashcard_id_foreign'),
        ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE', name='sm_two_flashcards_user_id_foreign'),
    ...
    repetitions_in_session: Mapped[int] = mapped_column(SmallInteger, nullable=False, server_default=text("'0'::smallint"))
    created_at: Mapped[Optional[datetime.datetime]] = mapped_column(TIMESTAMP(precision=0))
    updated_at: Mapped[Optional[datetime.datetime]] = mapped_column(TIMESTAMP(precision=0))
...
class StoryFlashcards(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['flashcard_id'], ['flashcards.id'], ondelete='CASCADE', name='story_flashcards_flashcard_id_foreign'),
        ForeignKeyConstraint(['story_id'], ['stories.id'], ondelete='CASCADE', name='story_flashcards_story_id_foreign'),
